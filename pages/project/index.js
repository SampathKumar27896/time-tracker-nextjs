...
const Project = ({ projectList, callBackendAPI }) => {
  ...
  const handlePageChange = async(page) => {
    ...
    const result = await callBackendAPI( { url: modifiedURL, method: GET_PROJECT.method});
    setListData(result);
  };
                ...
                <Row className="row">
                        <Col className="internal-page-header" span={24}>
                                <PageHeader
                                  ...
                                  extra={[
                                    <Button key="1" type="primary" onClick={() => {router.push('/project/addProject')}}>
                                      Add
                                    </Button>,
                                  ]}
                                />
                ...
                <Row className="row">
                        ...
                        <Col className="split-view-right" span={12}>
                                <Row className="row" justify="center">
                                  {(selectedItem && selectedItem.name)?  (<Card
...
export async function getServerSideProps({req}) {
  const result = await apiFetcher({url:`${process.env.API_DOMAIN_URL}/other/project/getProjects`, method: `GET`,
